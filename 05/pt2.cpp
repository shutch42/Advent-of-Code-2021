...
int** loadMap(line* head) {
	...
	while(curr->next != nullptr) {
		if(curr->start[0] == curr->end[0]) {
		...
		else if(curr->start[0] < curr->end[0] && curr->start[1] < curr->end[1]) {
			//up-right diagonal
			//cout << curr->start[0] << "," << curr->start[1] << " -> " << curr->end[0] << "," << curr->end[1] << endl;
			for(int i = 0; i <= curr->end[0] - curr->start[0]; i++) {
			//	cout << curr->start[0]+i << "," << curr->start[1]+i << endl;
				map[curr->start[0]+i][curr->start[1]+i]++;
			}
		}
		else if(curr->start[0] < curr->end[0] && curr->start[1] > curr->end[1]) {
			//down-right diagonal
			...
			//up-left diagonal
			//cout << curr->start[0] << "," << curr->start[1] << " -> " << curr->end[0] << "," << curr->end[1] << endl;
			for(int i = 0; i <= curr->start[0] - curr->end[0]; i++) {
			//	cout << curr->start[0]-i << "," << curr->start[1]+i << endl;
				map[curr->start[0]-i][curr->start[1]+i]++;
			}
...
int main() {
	...
	if(loadPts(input, head)) {
		...
		int x = getMaxX(head)+1;
		int y = getMaxY(head)+1;
		int total = getNumOverlap(map, x, y);
		cout << "There are " << total << " points of overlap" << endl;
	}
	else {
		exit(1);
	}
